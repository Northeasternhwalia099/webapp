...
public class UserController {
	...
	public ResponseEntity<?> userCreatEntity( @RequestBody(required = false) User uuser)
	{
		...
		if( (uuser.getFirstName()!=null && uuser.getFirstName().trim().length() >0) && (uuser.getLastName()!=null && uuser.getLastName().trim().length() >0) && uuser.getEmail()!=null && uuser.getPassword()!=null)
		{
		...
		//long end_t = System.currentTimeMillis();
		//statsDClient.recordExecutionTime("postUserApiTime", (end_t-startt_t));
		LOGGIN_LOGGER.error("Email, FirstName, LastName and Password all four fields cannot be null or First and Last Name cannot be blank");
		return new ResponseEntity<String>(entity.toString(), HttpStatus.BAD_REQUEST);
	}
	...
	public ResponseEntity<?> getUser(@PathVariable String id,HttpServletRequest request, HttpServletResponse response)
	{
		...
		if(autho != null && autho.toLowerCase().startsWith("basic"))
		{
            ...
            {
                return new ResponseEntity<String>("Please enter valid email address corresponding to ID" , HttpStatus.UNAUTHORIZED);
            }
	...
	public ResponseEntity<String> putUser( @PathVariable String id,@RequestBody(required = false) User uuser, HttpServletRequest request, HttpServletResponse response)
	{
		...
		if(autho != null && autho.toLowerCase().startsWith("basic"))
		{
			...
			{ 
				...
				if((uuser.getFirstName()!=null && uuser.getFirstName().trim().length() >0) && (uuser.getLastName()!=null && uuser.getLastName().trim().length() >0)  && uuser.getEmail()!=null && uuser.getPassword()!=null)
				{
					...
					else if(userRepository.findByemail(uuser.getEmail())!=null && !uuser.getEmail().equals(sUser.getEmail()))
					{
						//JsonObject jsonObject = new JsonObject();
